...
namespace WotDataLib
{
    ...
    static class ZipCache
    {
        ...
        /// </summary>
        public static Stream GetZipFileStream(string path)
        {
            var paths = path.Split('|');
            var zipfile = _cache.GetEntry(paths[0].ToLowerInvariant(), () => new ZipCacheEntry(paths[0])).Zip;
            if (zipfile == null)
                return null;
            var entry = zipfile.GetEntry(paths[1].Replace('\\', '/'));
            if (entry == null)
                return null;
            else
